...
type QAItem = {
  ...
  suggested_new_category?: { name: string; color: string; icon: string } | null;
};
...
export default function UploadPage() {
  ...
  const processFile = useCallback(async (entry: FileEntry, confirmOverage = false) => {
    ...
    try {
      ...
      // PDF multi-page: track all page statement IDs
      ...
      // Listen to SSE for each page, sum uncategorized counts
      const waitPage = (sid: number) => new Promise<number>((resolve, reject) => {
        const token = localStorage.getItem("token");
        const stream = new EventSource(
          `${process.env.NEXT_PUBLIC_API_URL || "/api/v1"}/statements/${sid}/progress?token=${token}`
        );
        stream.addEventListener("progress", (e) => {
          if (sid === primaryId) {
            const d = JSON.parse((e as MessageEvent).data);
            updateEntry(entry.id, { progress: { step: d.step, pct: d.pct, message: d.message } });
          }
        });
        stream.addEventListener("complete", (e) => {
          ...
          if (d.overlap_warning) {
            setOverlapWarnings(prev => [...prev, { file: entry.file.name, period: d.overlap_warning.period }]);
          }
          resolve(d.uncategorized_count ?? 0);
        });
        ...
        stream.onerror = () => { stream.close(); reject(new Error("Connection lost")); };
      });
      ...
      if (err.status === 402 && err.detail?.overage_confirmation_required) {
        // Business plan overage — pause and ask user to confirm
        updateEntry(entry.id, { status: "queued", progress: undefined });
        setOveragePending({
          file: entry.file,
          entryId: entry.id,
          overage_pages: err.detail.overage_pages,
          overage_cost_usd: err.detail.overage_cost_usd,
        });
        return;
      }
      // Hard quota exceeded or other error
      updateEntry(entry.id, {
        status: "error",
  ...
  const runQueue = useCallback(async (queue: FileEntry[]) => {
    ...
    // After all processed, collect QA items from successful statements
    setEntries(prev => {
      const doneEntries = prev.filter(e => e.status === "done");
      // If everything failed, stay on the upload page so error cards are visible
      ...
      Promise.all([
        ...
        // deduplicate by merchant_name
        const seen = new Set<string>();
        const unique = merged.filter((q: QAItem) => {
  ...
  const addFiles = useCallback((files: File[]) => {
    ...
    setEntries(prev => {
      ...
      // only start queue if not already running
    ...
    return (
      <div className="max-w-md mx-auto px-4 pt-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-4">
          ...
          <p className="text-sm text-slate-400 mb-4">{item.sample_description}</p>
          ...
          {item.suggested_new_category && (
            <div className="flex items-center gap-3 bg-violet-50 border border-violet-100 rounded-xl px-4 py-3 mb-4">
              <div className="w-7 h-7 rounded-full flex-shrink-0" style={{ backgroundColor: item.suggested_new_category.color }} />
              <div className="flex-1 min-w-0">
                <p className="text-xs text-violet-700 font-medium">AI suggests a new category</p>
                <p className="text-sm font-semibold text-violet-900 truncate">{item.suggested_new_category.name}</p>
              </div>
              <button
                onClick={() => handleAddAndUseNewCategory(item.suggested_new_category!)}
          ...
          {!item.suggested_new_category && item.suggested_category_name && (
            <p className="text-xs text-slate-400 mb-3">
              AI suggestion: <span className="text-emerald-600 font-medium">{item.suggested_category_name}</span>
            </p>
          )}

          <div className="grid grid-cols-3 gap-2 mb-3">
            {categories.filter(c => c.name !== "Uncategorized").map((cat: any) => (
        ...
        <div className="flex justify-center gap-1.5">
          {qaItems.map((_, i) => (
            <div key={i} className={`w-2 h-2 rounded-full ${i === qaIndex ? "bg-emerald-500" : i < qaIndex ? "bg-emerald-200" : "bg-gray-200"}`} />
          ))}
        </div>
      </div>
    );
  ...
  const confirmOverage = useCallback(async () => {
  ...
  }, [overagePending, entries, processFile]);
    ...
    <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        <div className="mb-5 bg-white rounded-2xl border border-gray-100 p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {usage.pages_remaining === 0 && (
              <Link href="/billing" className="text-xs font-semibold text-emerald-600 hover:underline">Upgrade plan →</Link>
            )}
          </div>
        </div>
      )}
      ...
      {quotaExhausted && !hasEntries && (
        <div className="mb-5 bg-red-50 border border-red-100 rounded-2xl p-5 text-center">
          ...
          <Link href="/billing" className="inline-block px-5 py-2.5 bg-red-500 text-white text-sm font-semibold rounded-xl hover:bg-red-600">
            Upgrade plan
          </Link>
        </div>
      )}
        ...
        <div className="mt-5 text-center">
          ...
          {entries.every(e => e.status === "error") && (
            <p className="mt-2 text-xs text-slate-400">
              Add valid API keys to <code className="bg-gray-100 px-1 rounded">backend/.env</code> to enable AI-powered OCR.
            </p>
          )}
        </div>
      )}
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-4">
      ...
      {entry.status === "error" && (
        <div className="mt-2 text-xs text-red-500 bg-red-50 px-3 py-2 rounded-lg">
          {entry.error}
          {entry.error?.includes("All models") && (
            <span className="block mt-1 text-red-400">Add API keys to backend/.env to enable processing.</span>
          )}
        </div>
      )}
    </div>
