...
export function UploadProvider({ children }: { children: React.ReactNode }) {
  ...
  // Only abort on provider unmount (logout/session end), not on navigation
  useEffect(() => {
          ...
          try {
            ...
            if (!receivedTerminal) {
              try {
                ...
                if (stmt.verify_status === "failed") { reject(new Error(formatVerifyErrors(stmt.verify_errors) ?? "Processing failed on the server")); return; }
                reject(new Error("Processing is still pending — refresh in a moment to see results.")); return;
              } catch {
                reject(new Error("Lost connection to the server while processing your file.")); return;
              }
            }
            resolve(0);
          } catch (err) {
  ...
  const runQueue = useCallback(async (queue: FileEntry[]) => {
    ...
    setEntries(prev => {
      ...
      Promise.all([Promise.all(successIds.map(sid => api.getQAPending(sid))), api.listCategories()]).then(([qaResults, cats]) => {
        ...
        setCategories(cats);
        if (unique.length > 0) { setQaItems(unique); setQaIndex(0); } else { setAllDone(true); }
      });
      return prev;
    });
  }, [processFile]);
